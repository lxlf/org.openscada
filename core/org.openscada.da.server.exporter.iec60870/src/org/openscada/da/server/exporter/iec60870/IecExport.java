...
public class IecExport
{
    ...
    public IecExport ( final String id, final Map<String, String> parameters, final ScheduledExecutorService executor, final ManageableObjectPool<DataItem> itemObjectPool, final HiveSource hiveSource )
    {
        ...
        final ObjectPoolDataItemFactory itemFactory = new ObjectPoolDataItemFactory ( executor, itemObjectPool, String.format ( "org.openscada.da.server.exporter.iec60870.export.%s.information.", id ) ); //$NON-NLS-1$
    ...
    private void checkAndCreate ()
    {
        ...
        {
            this.dataModule = new DataModule ( this.configuration.getDataModuleOptions (), new DataModelImpl ( this.hiveSource, this.configuration.getEntries (), this.configuration.getHiveProperties (), this.info, this.configuration.getSpontaneousBufferWindow (), this.configuration.getCyclicPeriod() ) );
