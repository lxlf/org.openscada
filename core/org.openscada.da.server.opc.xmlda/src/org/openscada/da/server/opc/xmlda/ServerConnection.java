...
public class ServerConnection implements SubscriptionListener
{
    ...
    protected synchronized void setItemValue ( final String clientHandle, final ItemValue value )
    {
        ...
        if ( value.getErrorInformation() != null )
        {
        	attributes.put ( "opc.error", Variant.TRUE );
        	attributes.put ( "opc.error.code", Variant.valueOf ( value.getErrorInformation ().getId ().getLocalPart () ) );
        	attributes.put ( "opc.error.message", Variant.valueOf ( value.getErrorInformation ().getMessage () ) );
        }
