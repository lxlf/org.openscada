...
public class SampleTestServerCallback
{
    ...
    private static void append ( final String fileName, final String data )
    {
        try
        {
            final PrintWriter pWriter = new PrintWriter ( new FileWriter ( fileName, true ) );
            pWriter.print ( data );
            pWriter.flush ();
            pWriter.close ();
        }
    ...
    public void UpdateMe ( final JIUnsignedShort size, final JIArray array )
    {
        ...
        final JIStruct[] structArray = (JIStruct[])array.getArrayInstance ();
    ...
    private static JILocalInterfaceDefinition registerInterface () throws JIException
    {
        ...
        final JILocalInterfaceDefinition interfaceDefinition = new JILocalInterfaceDefinition ( "D3F9CE10-686C-11d2-97BF-006008BD50B1", false );//IStatisUpdateMeSink

        final JIStruct VarData = new JIStruct ();// Will add in the struct later on
        ...
        final JIStruct NonVariableData = new JIStruct ();// Will add in the struct later on
        ...
        final JIArray NonVariableDataArray = new JIArray ( NonVariableData, null, 1, true );

        final JILocalParamsDescriptor updateParamObj = new JILocalParamsDescriptor ();
        ...
        final JILocalMethodDescriptor methodDescriptor = new JILocalMethodDescriptor ( "UpdateMe", updateParamObj );
        interfaceDefinition.addMethodDescriptor ( methodDescriptor );

        return interfaceDefinition;
    }

    public static void testStaticUpdateMeSink ( final String[] args ) throws JIException, InterruptedException, UnknownHostException
    {

        final JISession session = JISession.createSession ( args[1], args[2], args[3] );
        final JIComServer comStub = new JIComServer ( JIProgId.valueOf ( "TstMarsh.Test" ), args[0], session );
        final IJIComObject unknown = comStub.createInstance ();
        final IJIComObject ITest = unknown.queryInterface ( "89D8C8BE-1E91-11D3-910F-00C04F9403C2" ); //ITest

        //Create the Java Server class. This contains the instance to be called by the COM Server
        //
        final JILocalInterfaceDefinition interfaceDefinition = registerInterface ();
        if ( StaticSinkJavaCoClass == null )
        {
            StaticSinkJavaCoClass = new JILocalCoClass ( interfaceDefinition, new SampleTestServerCallback () );
        }
        final IJIComObject iStaticSink = JIObjectFactory.buildObject ( session, StaticSinkJavaCoClass );

        Object[] results = new Object[1];
        // Create the session
        final JICallBuilder javaCallback = new JICallBuilder ( true );
        ...
        final int staticSession = ( (Integer)results[0] ).intValue ();
    ...
    public static void testSinkDebug ( final String[] args ) throws JIException, InterruptedException, UnknownHostException
    {
